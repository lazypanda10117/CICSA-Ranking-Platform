    ...
    def abstractFormProcess(self, action, **kwargs):
        try:
            ...
            if dispatcher.get(action):
                ...
                event_tag = self.base_class.objects.get(id=event_tag_id);
            else:
                event_tag = self.base_class();

            event_tag.event_tag_event_id = [getSinglePostObj(post_dict, name + "_result") for name in self.search_name][0];
            event_tag.event_tag_name = getSinglePostObj(post_dict, 'event_tag_name');

            if not action == 'delete':
                event_tag.save();

            loghelper(self.request, 'admin', logQueryMaker(self.base_class, action.title(), id=event_tag.id));
...
### View Generating Functions
    ...
    def getSearchElement(self, **kwargs):
        def getSearchDefault(id):
            ...
            if element_id:
                event_tag = getModelObject(self.base_class, id=element_id);
                if event_tag.event_tag_event_id is not None:
                    linked_event = getModelObject(Event, id=event_tag.event_tag_event_id);
