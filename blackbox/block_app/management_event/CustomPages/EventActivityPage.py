...
from blackbox.api import EventAPI, EventActivityAPI, EventTagAPI, SchoolAPI, SummaryAPI, TeamAPI
...
class EventActivityPage(AbstractBasePage):
    def generateList(self):
        def genActivityDict(tag):
            event_activities = event_activity_api.filterEventActivity(
        ...
        def genTagDict(event_id):
            event_tags = event_tag_api.filterEventTag(event_tag_event_id=event_id);
        ...
        def genSummaryDict(event_id):
            event_summaries = summary_api.filterEventSummary(summary_event_parent=event_id);
        ...
        blockset = BlockSet().makeBlockSet(
            *[genActivityDict(event_tag) for event_tag in event_tag_api.filterEventTag(event_tag_event_id=event_id)], #event_activity_result
