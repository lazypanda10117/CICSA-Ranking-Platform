...
class EventActivityRankingPage(AbstractBasePage):
    def getPagePath(self):
        return 'ranking_management/eventActivityRace.html';
    ...
    def genPageObject(self):
        return dict(
            ...
            action_destination=reverse(
                'blackbox.block_app.management_ranking.process_dispatch_param',
        ...
        def getEventActivitySchool(event_activity_teams):
            return {school_team_tuple[0]: school_team_tuple[1]
                    for school_team_tuple in [
                        (lambda x: (api.SchoolAPI.getSchool(id=x.team_school), x))
                        (api.TeamAPI.getTeam(id=event_activity_team_id))
